...
<?php function print_carUser()
{
    $search_Contact = $_REQUEST['전화번호'];
    // echo $search_Contact;

    // $search_Contact_sql = sprintf('%s%s%s', '%', $search_Contact, '%');

    // echo $search_Contact_sql;
    // echo gettype($search_Contact);
    $conn = mysqli_connect('$ip_address', '$db_name', '$password', '$table_name', '$mysql_db_port');
    $result = mysqli_query(
        $connect,
        "SELECT * FROM 주차관리 WHERE 전화번호 LIKE '%$search_Contact%'"
    );
        ...
        <div class="page-container">
            ...
            <header class="header-desktop">
                ...
                <div class="section__content section__content--p30">
                    <div class="container-fluid">
                        <div class="header-wrap">
                            <form class="form-header" action="search_Contact.php" method="serch_Contact">
                                <input class="au-input au-input--xl" type="text" name="전화번호" placeholder="연락처 검색 (예시:000-0000-0000)" />
                                <button class="au-btn--submit" type="submit">
                                    <i class="zmdi zmdi-search"></i>
                                </button>
                            </form>
